...
The owner likes two things: this app's layout and flow, and the original website's calm, Airbnb-like look (warm paper, Fraunces serif headlines, Plus Jakarta Sans). So the look is a **skin**, separate from the layout. Both skins share the brand's accents — the cyan, magenta and yellow of the PRINTAIR wordmark (`public/brand/`), sampled from the artwork: cyan `#0094de`, magenta `#e6017f`, yellow `#f9c101`. Classic used to swap in the website's ember orange and teal; since the wordmark change there is no orange in the brand, and Classic keeps its warmth in the neutrals and type instead.

- Four palettes (`ink`, `paper`, `magenta`, `cyan`) and the two font families are CSS variables (`tailwind.config.js`). With no class you get **Process**. Inside a `.classic` wrapper the same class names resolve to the website's values: `ink` and `paper` turn warm, `font-display` becomes Fraunces and `font-sans` Plus Jakarta Sans. `magenta-*` and `cyan-*` are the same brand ramps in both skins.
- `src/styles/classic.css` holds what variables can't: the accent gradient button with its glow (keyed on `data-variant`, which `Button` sets), hairline inputs, white hairline option cards instead of coloured tiles (`data-option`, `data-option-icon`, `data-option-sub`, `data-option-tick`), accent progress segments (`data-progress-done` / `-todo`), warm sheet background (`data-sheet`), and hiding the press-room ornaments (`data-ornament`, halftones).
...
- To put another flow in Classic, pass `skin={useFlowSkin()}` to its `Sheet` (or wrap the page in a `div` with that class). Write components with the normal tokens; never hard-code a brand hex or Fraunces. Mark new option cards and ornaments with the data attributes above so both skins stay right.
