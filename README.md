...
- **Works on a weak connection.** The app shell, fonts and icons are saved on the device, so the app always opens. What you *read* (your projects, quotations, orders, profiles, the directories) is also kept per signed-in person as a "last seen" copy (`src/pwa/lastSeen.ts`): the network is always asked first, and the copy is shown only if the network fails or takes more than a few seconds, always with a strip saying when it was saved, and a Refresh button once the real answer lands. What you *send* (submitting, quoting, choosing, paying, uploading) is never saved, queued or replayed: it reaches the server now or fails with a plain message. Payment rows are never shown from a copy. Signing out erases it; copies expire after 7 days. The service worker itself still never caches Supabase, sign-in, files, edge functions or PayMongo (`vite.config.ts`).
